...
file_path = "./노지 작물 질병 진단 이미지"

image_path1 = "./노지 작물 질병 진단 이미지/Training/[원천]고추_0.정상"
image_path2 = "./노지 작물 질병 진단 이미지/Training/[원천]고추_질병"
image_path3 = "./노지 작물 질병 진단 이미지/Training/[원천]고추_9.증강"

label_path1 = "./노지 작물 질병 진단 이미지/Training/[라벨]고추_0.정상"
label_path2 = "./노지 작물 질병 진단 이미지/Training/[라벨]고추_1.질병"
label_path3 = "./노지 작물 질병 진단 이미지/Training/[라벨]고추_9.증강"
...
num = 0
for file in os.listdir(image_path3):
    path = image_path3 + "/" + file
    img = Image.open(path)
    img = img.resize((256,256))
    img_array = np.array(img)
    X.append(img_array)
...
for file in os.listdir(label_path1):
    ...
    if json_data["annotations"]["disease"] == 1:
        Y.append(0)
    elif json_data["annotations"]["disease"] == 2:
        Y.append(1)
    else:
        Y.append(2)
    # if num == 1000:
...
num = 0
for file in os.listdir(label_path3):
    dir_path = label_path3 + "/" + file
    with open(dir_path, "r", encoding="utf8") as f:
        contents = f.read() # string 타입
        json_data = json.loads(contents)
    if json_data["annotations"]["disease"] == 1:
...
model_gochu_dir = 'save_model/h5'
model_gochu_path = os.path.join(cur_dir, model_gochu_dir)
model_gochu_file = os.path.join(model_gochu_path, 'gochu_0523_50.h5')
os.makedirs(model_gochu_path, exist_ok = True)
model.save(model_gochu_file,save_format='h5')

# model.save('gochu.h5')
model.save_weights('gochu_weights_1.h5')
